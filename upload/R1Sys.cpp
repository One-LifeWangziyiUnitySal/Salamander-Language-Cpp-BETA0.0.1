...
// ---- 类系统 (Class) ----
// 类定义:  Class(类名, 成员变量1, 成员变量2, ...) ... EndClass
// 类体中用 Func 定义成员函数, 函数内用 this.field 访问成员
// 实例化:  obj = new(类名)
// 访问:    obj.field  obj.method(args)

struct ClassDef {
    std::string name;
    std::vector<std::string> fields;           // 成员变量名
    std::map<std::string, std::vector<std::string> > methods;  // 方法名 -> 代码行
    std::map<std::string, std::vector<std::string> > methodParams;  // 方法名 -> 参数名列表
};
std::map<std::string, ClassDef> g_classes;

// 对象实例: 存储为 dict (成员变量名 -> 值)
// 对象带 __class__ 字段记录类名
// 方法调用时设置 g_this 指向当前对象(dict名)

std::string g_this;  // 当前 this 指向的 dict 名 (空=无 this)

bool isClassDef(const std::string& name) {
    return g_classes.find(name) != g_classes.end();
}

// ---- 模块/命名空间系统 (Module) ----
// Module Name ... EndModule
//   模块内定义的 Func / 变量自动加前缀 "Name__"
//   访问: Name.func(args) / Name.varName  或  import Name 后直接使用
std::set<std::string> g_modules;          // 已注册的模块名
std::string g_currentModule;              // 当前所在模块 (空=全局)
...
// loadPack 实现 (在错误系统之后)
// 模块命名空间: 记录每个 Pack 加载时注册的函数名, 用于冲突检测
std::map<std::string, std::vector<std::string> > g_packExports;  // pack名 -> 导出的函数名列表

// loadPack 声明 (实现移到 g_funcs 定义之后)
bool loadPackImpl(const std::string& name);
...
// ---- º¯Êý¶¨Òå ----
// capturedVars: 闭包捕获的变量 (lambda创建时的局部变量快照)
//   - 调用时合并到函数作用域, 参数优先级高于捕获变量
struct Function {
    std::string name;
    std::vector<std::string> params;
    std::vector<std::string> body;
    std::map<std::string, std::string> capturedVars;  // 闭包捕获的变量
};
std::map<std::string, Function> g_funcs;

// loadPack 实现 (g_funcs 已定义)
bool loadPackImpl(const std::string& name) {
    ...
    std::vector<std::string> beforeFuncs;
    for (auto& f : g_funcs) beforeFuncs.push_back(f.first);
    runLines(lines);
    std::vector<std::string> newFuncs;
    for (auto& f : g_funcs) {
        bool existed = false;
        for (auto& b : beforeFuncs) { if (b == f.first) { existed = true; break; } }
        if (!existed) {
            std::string nsName = name + "::" + f.first;
            g_funcs[nsName] = f.second;
            newFuncs.push_back(f.first);
        }
    }
    g_packExports[name] = newFuncs;
    Log("[Pack] loaded: " + name + " (" + std::to_string(lines.size()) + " lines, " +
        std::to_string(newFuncs.size()) + " funcs)");
    return true;
}
...
static void expandModuleAccess(std::string& s);  // 模块访问展开前向声明
...
double calc(const std::string& e) {
    ...
    if (!g_currentModule.empty() && isIdentifier(s)) {
        std::string p = g_currentModule + "__" + s;
        if (hasVar(p)) return atof(getVar(p).c_str());
    }
...
bool cond(const std::string& c) {
    ...
    // 逻辑与 && (找顶层 &&)
    {
        ...
        for (size_t i = 0; i + 1 < s.size(); i++) {
            ...
            if (depth == 0 && ch == '&' && s[i+1] == '&') {
                return cond(s.substr(0, i)) && cond(s.substr(i + 2));
            }
        }
    }
...
std::string callFunc(const std::string& name, const std::vector<std::string>& args) {
    ...
    std::string inferredModule;
    {
        size_t pos = name.find("__");
        if (pos != std::string::npos && pos > 0) {
            std::string prefix = name.substr(0, pos);
            if (g_modules.count(prefix)) inferredModule = prefix;
        }
    }
    std::string savedModule = g_currentModule;
    g_currentModule = inferredModule;
    ...
    std::set<std::string> closureAndParams;
    ...
    // 闭包: 注入捕获的变量 (在参数绑定之前, 这样参数可覆盖同名捕获变量)
    for (auto& cv : f.capturedVars) {
        g_vars[cv.first] = cv.second;
    }
    ...
    // 恢复: 保留对已存在全局变量 (非闭包、非参数) 的修改; 清除局部变量
    g_returning = savedReturning;
    g_breakLoop = savedBreak;
    for (auto& vp : g_vars) {
        if (savedGlobalVars.count(vp.first) && !closureAndParams.count(vp.first)) {
            savedGlobalVars[vp.first] = vp.second;
        }
    }
...
// ---- 运算符重载 ----
// 检查表达式是否含顶层算术运算符(+,-,*,/)且操作数为对象
// 若对象的类定义了 __add__/__sub__/__mul__/__div__ 方法, 则调用之
// 成功时设置 result 并返回 true
static bool tryOperatorOverload(const std::string& expr, std::string& result) {
    std::string s = expr;
    trim(s);
    if (s.empty()) return false;
    ...
    if (findTopLevelOp(s, '+', '-', pos)) {
        op = s[pos];
    } else if (findTopLevelMulDiv(s, '*', pos)) {
        op = '*';
    } else if (findTopLevelMulDiv(s, '/', pos)) {
        op = '/';
    }
    ...
    // 求值两边 (检查是否是对象)
    std::string lv = evalExpr(lhs);
    if (g_waitingInput) return false;
    std::string rv = evalExpr(rhs);
    if (g_waitingInput) return false;

    // 检查左操作数是否是对象实例 (dict 含 __class__)
    std::string objDictName, className, otherVal;
    if (!lv.empty() && isDictVar(lv)) {
        auto& d = getDict(lv);
        auto it = d.find("__class__");
        if (it != d.end()) { objDictName = lv; className = it->second; otherVal = rv; }
    }
    // 左边不是对象, 检查右操作数
    if (objDictName.empty() && !rv.empty() && isDictVar(rv)) {
        auto& d = getDict(rv);
        auto it = d.find("__class__");
        if (it != d.end()) { objDictName = rv; className = it->second; otherVal = lv; }
    }
    if (objDictName.empty() || className.empty()) return false;
    if (!isClassDef(className)) return false;
    ...
    // 调用方法: this = objDictName, 第一个参数 = otherVal
    std::string savedThis = g_this;
    g_this = objDictName;
    std::map<std::string, std::string> savedVars = g_vars;
    auto& params = cls.methodParams[methodName];
    for (size_t ai = 0; ai < params.size(); ai++) {
        setVar(params[ai], ai == 0 ? otherVal : "");
    }
    setVar("__ret__", "");
    ...
    g_callDepth++;
    std::string savedFuncName = g_currentFuncName;
    g_currentFuncName = className + "." + methodName;
...
std::string evalExpr(const std::string& expr) {
    ...
    // 模块访问展开: Module.func() -> Module__func(), Module.var -> Module__var
    ...
    if (isIdentifier(s)) {
        if (!g_currentModule.empty()) {
            std::string p = g_currentModule + "__" + s;
            if (hasVar(p)) return getVar(p);
            if (isListVar(p)) return p;
            if (isDictVar(p)) return p;
        }
    ...
    // this 关键字
    if (s == "this") {
        if (g_this.empty()) { reportError(ERR_UNDEFINED_VAR, "this used outside method"); return ""; }
        return g_this;
    }

    // this.field 访问成员变量 (仅当 field 是简单标识符, 不含运算符)
    if (s.size() > 5 && s[0] == 't' && s[1] == 'h' && s[2] == 'i' && s[3] == 's' && s[4] == '.' && !g_this.empty()) {
        std::string field = s.substr(5);
        trim(field);
        if (field.find_first_of("+-*/=()[]") == std::string::npos && isDictVar(g_this)) {
            auto& d = getDict(g_this);
            auto it = d.find(field);
            std::string val = (it != d.end()) ? it->second : "";
            return val;
        }
    }

    // obj.field 访问对象成员变量 (obj 是变量名,含点号)
    {
        size_t dot = s.find('.');
        if (dot != std::string::npos && dot > 0 && dot < s.size() - 1) {
            // 检查是否是 obj.field 模式 (不含运算符)
            std::string leftPart = s.substr(0, dot);
            std::string rightPart = s.substr(dot + 1);
            // 如果右侧有运算符或括号,不是简单的字段访问
            if (rightPart.find_first_of("+-*/=()[]") == std::string::npos &&
                leftPart.find_first_of("+-*/=()[]") == std::string::npos) {
                // 获取对象名
                std::string objName;
                ...
                if (!objName.empty() && isDictVar(objName)) {
                    auto& d = getDict(objName);
                    auto it = d.find(rightPart);
    ...
    while (replaced && safetyCount++ < 1000) {
        ...
        if (!replaced) {
            ...
            static const char* builtins[] = {
                ...
                "gc","memStats","new",
                "tan","atan","exp","log10","clamp","lerp","dist",
                "isNum","isStr","isList","isDict","isFunc","isClass","type",
                ...
                "iter","hasNext","next","iterValue", NULL
            };
            for (int bi = 0; builtins[bi]; bi++) {
                ...
                while (pos != std::string::npos) {
                    ...
                    bool noEval = (strcmp(builtins[bi], "getCbox") == 0 || strcmp(builtins[bi], "isCbox") == 0 || strcmp(builtins[bi], "new") == 0);
                    std::string trimmedArgs = argsStr; trim(trimmedArgs);
        ...
        // 变量作为函数调用: f(args) where f is a var containing function name (闭包调用)
        if (!replaced) {
            for (size_t pos = 0; pos < s.size(); pos++) {
                if (s[pos] == '(' && pos > 0) {
                    ...
                    std::string varName = s.substr(nameStart, nameEnd - nameStart);
                    if (!isIdentifier(varName) || !hasVar(varName)) continue;
                    std::string funcName = getVar(varName);
                    ...
                    std::string fr = callFunc(funcName, cargs);
                    if (g_waitingInput) return "";
                    s.replace(nameStart, rp - nameStart + 1, fr); replaced = true; break;
                }
            }
        }
        // Lambda: lambda(params, body) -> 创建匿名函数
        // 语法: lambda("a, b", "return a + b")
        // 返回函数名, 可直接调用: lambda("a,b", "return a+b")(3, 4)
        if (!replaced) {
            size_t lpos = s.find("lambda(");
            if (lpos != std::string::npos) {
                if (lpos == 0 || (lpos > 0 && !isalnum((unsigned char)s[lpos-1]) && s[lpos-1] != '_')) {
                    size_t lp = lpos + 7;
                    int depth = 1; size_t rp = lp;
                    bool inS = false, inD = false;
                    while (rp < s.size() && depth > 0) {
                    ...
                    if (depth == 0) {
                        std::string argsStr = s.substr(lp, rp - lp);
                        std::vector<std::string> largs; size_t lap = 0;
                        while (lap <= argsStr.size()) {
                            std::string a = getArg(argsStr, lap);
                            largs.push_back(evalExpr(a));
                            if (lap < argsStr.size() && argsStr[lap] == ',') lap++;
                            else break;
                        }
                        // largs[0] = 参数列表(逗号分隔), largs[1] = 函数体
                        if (largs.size() >= 2) {
                            static int lambdaCounter = 0;
                            lambdaCounter++;
                            std::string fnName = "__lambda_" + std::to_string(lambdaCounter) + "__";
                            Function lf;
                            lf.name = fnName;
                            // 解析参数
                            std::istringstream iss(largs[0]);
                            std::string pn;
                            while (std::getline(iss, pn, ',')) {
                                trim(pn);
                                if (!pn.empty()) lf.params.push_back(pn);
                            }
                            // 解析函数体(按行分割)
                            std::istringstream biss(largs[1]);
                            std::string line;
                            while (std::getline(biss, line)) {
                                trim(line);
                                if (!line.empty()) lf.body.push_back(line);
                            }
                            // 闭包: 捕获当前作用域的所有变量 (快照)
                            lf.capturedVars = g_vars;
                            g_funcs[fnName] = lf;
                            s.replace(lpos, rp - lpos + 1, fnName);
                            replaced = true;
                        }
                    }
                }
            ...
            for (auto& clsPair : g_classes) {
                for (auto& methodPair : clsPair.second.methods) {
                    ...
                    while (pos != std::string::npos) {
                        ...
                        while (nameStart > 0 && (isalnum((unsigned char)s[nameStart - 1]) || s[nameStart - 1] == '_')) nameStart--;
                        std::string objVarName = s.substr(nameStart, pos - 1 - nameStart);
                        if (objVarName.empty()) { pos = s.find(pattern, pos + 1); continue; }
                        // 获取对象 dict 名
                        std::string objDictName;
                        ...
                        size_t lp = pos + pattern.length();
                        int depth = 1; size_t rp = lp;
                        while (rp < s.size() && depth > 0) { char c = s[rp]; if (c == '(') depth++; else if (c == ')') { depth--; if (depth == 0) break; } rp++; }
                        if (depth != 0) break;
                        // 解析参数
                        std::string argsStr = s.substr(lp, rp - lp);
                        std::vector<std::string> cargs; size_t ap = 0;
                        ...
                        g_this = objDictName;
                        std::map<std::string, std::string> savedVars = g_vars;
                        // 使用方法定义中的参数名绑定, 不是 p0/p1
                        auto& params = clsPair.second.methodParams[methodPair.first];
                        for (size_t ai = 0; ai < cargs.size() && ai < params.size(); ai++) {
                            setVar(params[ai], cargs[ai]);
                        }
                        setVar("__ret__", "");
                        bool savedRet = g_returning; g_returning = false;
                        bool savedBreak = g_breakLoop; g_breakLoop = false;
                        g_callDepth++;
                        std::string savedFuncName = g_currentFuncName;
                        g_currentFuncName = clsPair.first + "." + methodPair.first;
                        runLines(methodPair.second);
                        std::string fr = getVar("__ret__");
                        g_returning = savedRet; g_breakLoop = savedBreak;
                        g_vars = savedVars;
                        g_currentFuncName = savedFuncName;
                        g_callDepth--;
                        g_this = savedThis;
                        // 替换
                        s.replace(nameStart, rp - nameStart + 1, fr);
                        replaced = true;
                        break;
                    }
                    if (replaced) break;
        ...
        while (didReplace && safety++ < 1000) { didReplace = false;
            for (size_t p = 0; p < s.size(); p++) {
                if (s[p] == '.' && p > 0) {
                    ...
                    while (nameStart > 0 && (isalnum((unsigned char)s[nameStart-1]) || s[nameStart-1] == '_')) nameStart--;
                    if (nameStart >= p) continue;
                    std::string objName2 = s.substr(nameStart, p - nameStart);
                    ...
                    std::string field = s.substr(p + 1, fieldEnd - p - 1);
                    std::string odn;
    ...
    if (hasPlus) {
        ...
        if (findConcatPlus(s, pp)) {
            ...
            std::string lv = evalExpr(lt), rv = evalExpr(rt);
            if (g_waitingInput) return "";
...
std::string resolveFuncArg(const std::string& raw) {
    ...
    if (isIdentifier(s)) {
        ...
        if (!g_currentModule.empty()) {
            std::string prefixed = g_currentModule + "__" + s;
...
// 模块访问展开: 将 "ModuleName.member" 翻译为 "ModuleName__member"
// 仅当 ModuleName 是已注册模块时替换, 避免影响 obj.field 访问
// 跳过引号内的内容
static void expandModuleAccess(std::string& s) {
    ...
    for (size_t i = 0; i < s.size(); i++) {
        ...
        if (c == '.') {
            ...
            while (nameStart > 0 && (isalnum((unsigned char)s[nameStart-1]) || s[nameStart-1] == '_'))
                nameStart--;
            if (nameStart >= nameEnd) continue;
            std::string modName = s.substr(nameStart, nameEnd - nameStart);
    ...
    if (!g_currentModule.empty()) {
        std::string prefix = g_currentModule + "__";
        for (auto& fp : g_funcs) {
            if (fp.first.size() > prefix.size() &&
                fp.first.compare(0, prefix.size(), prefix) == 0) {
                std::string shortName = fp.first.substr(prefix.size());
                std::string pat = shortName + "(";
                size_t pos = 0;
...
bool builtinCall(const std::string& name, const std::vector<std::string>& args, std::string& result) {
    ...
    if (name == "listNew") {
        ...
        if (g_inModuleBody) lname = g_currentModule + "__" + lname;
    ...
    // new(类名) - 创建对象实例, 返回对象名(dict名)
    if (name == "new") {
        if (args.empty()) { reportError(ERR_ARG_COUNT, "new needs class name"); result = ""; return true; }
        std::string className = args[0];
        if (!isClassDef(className)) {
            reportError(ERR_UNDEFINED_VAR, "undefined class: " + className);
            result = ""; return true;
        }
        ClassDef& cls = g_classes[className];
        // 生成唯一对象名
        ...
        objCounter++;
        std::string objName = "__obj_" + std::to_string(objCounter) + "__";
        // 创建 dict 存储成员变量
        std::map<std::string, std::string> objData;
        objData["__class__"] = className;
        for (auto& f : cls.fields) objData[f] = "";
        g_dicts[objName] = objData;
        result = objName;
        return true;
    }
    ...
    // tan(n) - 正切
    if (name == "tan") { if(args.empty()){result="0";return true;} char b[64];sprintf(b,"%g",tan(atof(args[0].c_str()))); result=b; return true; }
    // atan(n) - 反正切
    if (name == "atan") { if(args.empty()){result="0";return true;} char b[64];sprintf(b,"%g",atan(atof(args[0].c_str()))); result=b; return true; }
    // exp(n) - e的n次方
    if (name == "exp") { if(args.empty()){result="0";return true;} char b[64];sprintf(b,"%g",exp(atof(args[0].c_str()))); result=b; return true; }
    // log10(n) - 以10为底对数
    if (name == "log10") { if(args.empty()){result="0";return true;} double v=atof(args[0].c_str()); if(v<=0){result="0";return true;} char b[64];sprintf(b,"%g",log10(v)); result=b; return true; }
    // clamp(n, min, max) - 限制范围
    if (name == "clamp") {
        if(args.size()<3){result="0";return true;}
        double v=atof(args[0].c_str()),lo=atof(args[1].c_str()),hi=atof(args[2].c_str());
        char b[64]; sprintf(b,"%g",v<lo?lo:(v>hi?hi:v)); result=b; return true;
    }
    // lerp(a, b, t) - 线性插值
    if (name == "lerp") {
        if(args.size()<3){result="0";return true;}
        double a=atof(args[0].c_str()),bv=atof(args[1].c_str()),t=atof(args[2].c_str());
        char b[64]; sprintf(b,"%g",a+(bv-a)*t); result=b; return true;
    }
    // dist(x1, y1, x2, y2) - 两点距离
    if (name == "dist") {
        if(args.size()<4){result="0";return true;}
        double dx=atof(args[0].c_str())-atof(args[2].c_str());
        double dy=atof(args[1].c_str())-atof(args[3].c_str());
        char b[64]; sprintf(b,"%g",sqrt(dx*dx+dy*dy)); result=b; return true;
    }
    ...
    if (name == "type") {
        if(args.empty()){result="null";return true;}
        ...
        if(isDictVar(args[0])){result="dict";return true;}
        if(hasVar(args[0])){result="var";return true;}
        result="string";return true;
    }

    // ============ 文件操作 ============
    if (name == "fileRead") {
        if(args.empty()){reportError(ERR_ARG_COUNT,"fileRead needs path");result="";return true;}
        FILE* fp=fopen(args[0].c_str(),"rb");
        if(!fp){reportError(ERR_FILE_IO,"cannot read file",args[0]);result="";return true;}
        fseek(fp,0,SEEK_END);long sz=ftell(fp);fseek(fp,0,SEEK_SET);
        if(sz<0)sz=0; if(sz>1024*1024)sz=1024*1024;
        std::vector<char> buf(sz+1,0);
        fread(buf.data(),1,sz,fp);fclose(fp);
        // skip SALF1 header
        char* start=buf.data();
        if(sz>=5&&strncmp(start,"SALF1",5)==0)start+=5;
        result=std::string(start);return true;
    }
    if (name == "fileWrite") {
        if(args.size()<2){reportError(ERR_ARG_COUNT,"fileWrite needs path and content");result="";return true;}
        FILE* fp=fopen(args[0].c_str(),"wb");
        if(!fp){reportError(ERR_FILE_IO,"cannot write file",args[0]);result="";return true;}
        fwrite(args[1].c_str(),1,args[1].size(),fp);
        fclose(fp);
        result="true";return true;
    }
    if (name == "fileAppend") {
        if(args.size()<2){reportError(ERR_ARG_COUNT,"fileAppend needs path and content");result="";return true;}
        FILE* fp=fopen(args[0].c_str(),"ab");
        if(!fp){reportError(ERR_FILE_IO,"cannot append file",args[0]);result="";return true;}
        fwrite(args[1].c_str(),1,args[1].size(),fp);
        fclose(fp);
        result="true";return true;
    }
    ...
    // ============ sleep 函数 ============
    if (name == "sleep") {
        if(args.empty()){result="true";return true;}
    ...
    // [e1, e2, e3] - 创建列表
    if (name == "list") {
        std::string listName="__lit_list_"+std::to_string(g_lists.size())+"__";
        setList(listName, args);
        result=listName;return true;
    }
    // {k1:v1, k2:v2} - 创建字典 (通过 dictFrom 函数)
    // 由于 {} 会被解析器特殊处理, 用 dict(k1, v1, k2, v2) 替代
    if (name == "dict") {
        std::string dictName="__lit_dict_"+std::to_string(g_dicts.size())+"__";
        std::map<std::string,std::string> d;
        for(size_t i=0;i+1<args.size();i+=2) d[args[i]]=args[i+1];
        g_dicts[dictName]=d;
        result=dictName;return true;
    }

    // ============ 错误信息 ============
    // lastError() - 获取最后一个错误信息
    if (name == "lastError") {
        if (g_errors.empty()) { result = ""; return true; }
        result = g_errors.back().message;
        return true;
    }
    // lastErrorCode() - 获取最后一个错误码
    if (name == "lastErrorCode") {
        if (g_errors.empty()) { result = "0"; return true; }
        result = std::to_string(g_errors.back().code);
        return true;
    }
    // clearErrors() - 清除所有错误
    if (name == "clearErrors") {
    ...
    // errorCount() - 获取错误数量
    if (name == "errorCount") {
        result = std::to_string(g_errors.size());
        return true;
    }
    ...
    if (name == "dictNew") {
        ...
        if (g_inModuleBody) dn = g_currentModule + "__" + dn;
    ...
    // ============ 迭代器 (Iterator) ============
    // iter(collection) - 创建迭代器 (支持 list 和 dict)
    //   返回一个 dict: {__type__: "list"/"dict", __data__: 数据名, __keys__: 键列表(仅dict), __pos__: 0}
    static int iterCounter = 0;
    if (name == "iter") {
        if (args.empty()) { reportError(ERR_INVALID_ARG, "iter needs 1 arg", name); result = ""; return true; }
        std::string src = args[0];
        iterCounter++;
        std::string iterName = "__iter_" + std::to_string(iterCounter) + "__";
        std::map<std::string, std::string> it;
        if (isListVar(src)) {
            // 列表迭代器: 复制数据到内部列表
            std::string dataListName = iterName + "_data";
            setList(dataListName, getList(src));
            it["__type__"] = "list";
            it["__data__"] = dataListName;
            it["__pos__"] = "0";
        } else if (isDictVar(src)) {
            // 字典迭代器: 复制数据 + 键列表
            std::string dataDictName = iterName + "_data";
            setDict(dataDictName, getDict(src));
            std::string keysListName = iterName + "_keys";
            std::vector<std::string> keys;
            for (auto& kv : getDict(src)) keys.push_back(kv.first);
            setList(keysListName, keys);
            it["__type__"] = "dict";
            it["__data__"] = dataDictName;
            it["__keys__"] = keysListName;
            it["__pos__"] = "0";
        } else {
            reportError(ERR_INVALID_TYPE, "iter requires list or dict, got: " + src, name);
            result = "";
            return true;
        }
        setDict(iterName, it);
    ...
    // hasNext(iter) - 是否还有下一个元素
    if (name == "hasNext") {
        if (args.empty()) { result = "false"; return true; }
        if (!isDictVar(args[0])) { result = "false"; return true; }
        auto& it = getDict(args[0]);
        std::string type = it.count("__type__") ? it["__type__"] : "";
        int pos = it.count("__pos__") ? atoi(it["__pos__"].c_str()) : 0;
        if (type == "list") {
            std::string dataName = it["__data__"];
            if (!isListVar(dataName)) { result = "false"; return true; }
            result = (pos < (int)getList(dataName).size()) ? "true" : "false";
        } else if (type == "dict") {
            std::string keysName = it["__keys__"];
            if (!isListVar(keysName)) { result = "false"; return true; }
            result = (pos < (int)getList(keysName).size()) ? "true" : "false";
        } else {
            result = "false";
        }
        return true;
    }
    // next(iter) - 获取下一个元素 (list: 元素值; dict: 键) 并推进位置
    if (name == "next") {
        if (args.empty()) { reportError(ERR_INVALID_ARG, "next needs 1 arg", name); result = ""; return true; }
        if (!isDictVar(args[0])) { reportError(ERR_INVALID_TYPE, "next requires iterator", name); result = ""; return true; }
        auto& it = getDict(args[0]);
        std::string type = it.count("__type__") ? it["__type__"] : "";
        int pos = it.count("__pos__") ? atoi(it["__pos__"].c_str()) : 0;
        if (type == "list") {
            std::string dataName = it["__data__"];
            if (!isListVar(dataName)) { reportError(ERR_INTERNAL, "iterator data lost", name); result = ""; return true; }
            auto& lst = getList(dataName);
            if (pos < 0 || pos >= (int)lst.size()) { reportError(ERR_OUT_OF_RANGE, "iterator exhausted", name); result = ""; return true; }
            result = lst[pos];
            it["__pos__"] = std::to_string(pos + 1);
        } else if (type == "dict") {
            std::string keysName = it["__keys__"];
            std::string dataName = it["__data__"];
            if (!isListVar(keysName) || !isDictVar(dataName)) { reportError(ERR_INTERNAL, "iterator data lost", name); result = ""; return true; }
            auto& keys = getList(keysName);
            if (pos < 0 || pos >= (int)keys.size()) { reportError(ERR_OUT_OF_RANGE, "iterator exhausted", name); result = ""; return true; }
            result = keys[pos];  // 返回键
            it["__pos__"] = std::to_string(pos + 1);
        } else {
            reportError(ERR_INVALID_TYPE, "not an iterator", name);
            result = "";
        }
        return true;
    }
    // iterValue(iter) - 获取当前迭代位置的值 (仅dict: 返回键对应的值)
    if (name == "iterValue") {
        if (args.empty()) { result = ""; return true; }
        if (!isDictVar(args[0])) { result = ""; return true; }
        auto& it = getDict(args[0]);
        ...
        if (type == "dict") {
            std::string keysName = it["__keys__"];
            std::string dataName = it["__data__"];
            if (isListVar(keysName) && isDictVar(dataName)) {
                auto& keys = getList(keysName);
                // pos 已推进, 取 pos-1 的键
                int idx = pos - 1;
                if (idx >= 0 && idx < (int)keys.size()) {
                    auto& d = getDict(dataName);
                    auto kv = d.find(keys[idx]);
                    result = (kv != d.end()) ? kv->second : "";
                }
            }
        } else if (type == "list") {
            std::string dataName = it["__data__"];
            if (isListVar(dataName)) {
                auto& lst = getList(dataName);
                int idx = pos - 1;
                if (idx >= 0 && idx < (int)lst.size()) result = lst[idx];
            }
        }
        return true;
    }
...
void runCode(const std::string& input) {
    ...
    if (startsWith(s, "return ")) {
        ...
        trim(val);
        std::string resolved = resolveFuncArg(val);
        setVar("__ret__", resolved);
        g_returning = true;
        return;
    }
    if (s == "return") {
    ...
    if (eqPos != std::string::npos && eqPos > 0 &&
        ...
        // 检查是否是 obj.field = val 或 this.field = val 赋值
        {
            size_t dot = vn.find('.');
            if (dot != std::string::npos && dot > 0 && dot < vn.size() - 1) {
                std::string leftPart = vn.substr(0, dot);
                std::string field = vn.substr(dot + 1);
                trim(leftPart); trim(field);
                std::string objName;
                if (leftPart == "this") objName = g_this;
                else if (hasVar(leftPart)) objName = getVar(leftPart);
                if (!objName.empty() && isDictVar(objName)) {
        ...
        }
        {
            ...
            // 模块内变量自动加前缀
            if (g_inModuleBody && vn.find("__") == std::string::npos) vn = g_currentModule + "__" + vn;
            setVar(vn, val);
        }
        return;
    }
    ...
    if (parenPos != std::string::npos && s.back() == ')') {
        ...
        // obj.method(args) 或 this.method(args) 方法调用
        {
            size_t dot = fn.find('.');
            if (dot != std::string::npos && dot > 0 && dot < fn.size() - 1) {
                std::string leftPart = fn.substr(0, dot);
                std::string methodName = fn.substr(dot + 1);
                trim(leftPart); trim(methodName);
                std::string objName;
                if (leftPart == "this") objName = g_this;
                else if (hasVar(leftPart)) objName = getVar(leftPart);
                if (!objName.empty() && isDictVar(objName)) {
                    auto& d = getDict(objName);
                    auto classIt = d.find("__class__");
                    if (classIt != d.end() && isClassDef(classIt->second)) {
                        ClassDef& cls = g_classes[classIt->second];
                        auto methodIt = cls.methods.find(methodName);
                        if (methodIt != cls.methods.end()) {
                            std::vector<std::string> args;
                            for (size_t ai = 0; ai < rawArgs.size(); ai++) {
                                args.push_back(resolveFuncArg(rawArgs[ai]));
                                if (g_waitingInput) return;
                            }
                            std::string savedThis = g_this;
                            g_this = objName;
                            ...
                            // 使用方法定义中的参数名绑定
                            auto& params = cls.methodParams[methodName];
                            for (size_t ai = 0; ai < args.size() && ai < params.size(); ai++) {
                                setVar(params[ai], args[ai]);
                            }
                            setVar("__ret__", "");
                            bool savedRet = g_returning; g_returning = false;
                            bool savedBreak = g_breakLoop; g_breakLoop = false;
                            if (g_callDepth >= MAX_CALL_DEPTH) {
                                reportError(ERR_STACK_OVERFLOW, "call depth exceeded", methodName);
                                g_this = savedThis; return;
                            }
                            g_callDepth++;
                            std::string savedFuncName = g_currentFuncName;
                            g_currentFuncName = cls.name + "." + methodName;
                            runLines(methodIt->second);
                            g_returning = savedRet;
                            g_breakLoop = savedBreak;
                            g_vars = savedVars;
                            g_currentFuncName = savedFuncName;
                            g_callDepth--;
...
void runLines(const std::vector<std::string>& lines) {
    for (size_t i = 0; i < lines.size(); i++) {
        ...
        // 模块内定义的 Func / 变量自动加前缀 "模块名__"
        // 实现: 设置 g_currentModule, 逐行执行模块体, EndModule 时清除
        if (startsWith(s, "Module(")) {
            size_t p1 = s.find('('), p2 = s.find_last_of(')');
            if (p1 != std::string::npos && p2 != std::string::npos && p2 > p1) {
                std::string modName = s.substr(p1 + 1, p2 - p1 - 1);
                trim(modName);
                modName = resolveFuncArg(modName);
                if (!modName.empty()) {
                    // 嵌套模块: 拼接前缀 (Outer__Inner)
                    std::string fullModName = modName;
                    if (!g_currentModule.empty()) fullModName = g_currentModule + "__" + modName;
                    g_modules.insert(fullModName);
                    std::string savedModule = g_currentModule;
                    g_currentModule = fullModName;
                    bool savedInModuleBody = g_inModuleBody;
                    g_inModuleBody = true;
                    // 收集模块体直到 EndModule (支持嵌套)
                    int depth = 0;
                    std::vector<std::string> modBody;
                    for (size_t j = i + 1; j < lines.size(); j++) {
                        ...
                        modBody.push_back(lines[j]);
                        if (j == lines.size() - 1) i = j;
                    }
                    runLines(modBody);
                    g_currentModule = savedModule;
                    g_inModuleBody = savedInModuleBody;
                    Log("[Module] " + fullModName + " registered");
                }
            }
            continue;
        }

        // import 模块名 — 将模块内所有函数/变量导入当前作用域 (创建别名)
        if (startsWith(s, "import ")) {
            std::string modName = s.substr(7);
            trim(modName);
            std::string prefix = modName + "__";
            // 导入函数别名
            std::vector<std::pair<std::string, Function> > newFuncs;
            for (auto& fp : g_funcs) {
                if (fp.first.size() > prefix.size() &&
                    fp.first.compare(0, prefix.size(), prefix) == 0) {
                    std::string shortName = fp.first.substr(prefix.size());
                    Function alias = fp.second;
                    alias.name = shortName;
                    newFuncs.push_back(std::make_pair(shortName, alias));
                }
            }
            for (auto& nf : newFuncs) g_funcs[nf.first] = nf.second;
            // 导入变量别名
            std::map<std::string, std::string> newVars;
            for (auto& vp : g_vars) {
                if (vp.first.size() > prefix.size() &&
                    vp.first.compare(0, prefix.size(), prefix) == 0) {
                    std::string shortName = vp.first.substr(prefix.size());
                    newVars[shortName] = vp.second;
                }
            }
            for (auto& nv : newVars) g_vars[nv.first] = nv.second;
            Log("[import] " + modName + " (" + std::to_string(newFuncs.size()) + " funcs, " +
                std::to_string(newVars.size()) + " vars)");
            continue;
        }

        // Class(类名, 成员变量...) ... EndClass
        // 类体中用 Func 定义方法, this.field 访问成员
        if (startsWith(s, "Class(")) {
            size_t p1 = s.find('('), p2 = s.find_last_of(')');
            if (p1 == std::string::npos || p2 == std::string::npos) continue;
            std::string as = s.substr(p1 + 1, p2 - p1 - 1);
            std::vector<std::string> ca; size_t ap = 0;
            while (ap <= as.size()) {
                std::string a = getArg(as, ap);
                ca.push_back(a);
            ...
            ClassDef cls;
            cls.name = ca[0];
            for (size_t k = 1; k < ca.size(); k++) {
                std::string fn = ca[k]; trim(fn);
                if (!fn.empty()) cls.fields.push_back(fn);
            }
            // 收集类体, 解析 Func 方法定义
            int depth = 0;
            std::string curMethodName;
            std::vector<std::string> curMethodParams;
            std::vector<std::string> curMethodBody;
            bool inMethod = false;
            for (size_t j = i + 1; j < lines.size(); j++) {
                std::string l = lines[j]; trim(l);
                ...
                if (startsWith(l, "Func(") && depth == 0) {
                    // 开始方法定义
                    if (inMethod) {
                        cls.methods[curMethodName] = curMethodBody;
                        cls.methodParams[curMethodName] = curMethodParams;
                        curMethodBody.clear();
                        curMethodParams.clear();
                    }
                    size_t mp1 = l.find('('), mp2 = l.find_last_of(')');
                    if (mp1 != std::string::npos && mp2 != std::string::npos) {
                        std::string mas = l.substr(mp1 + 1, mp2 - mp1 - 1);
                        std::vector<std::string> mfa; size_t map = 0;
                        while (map <= mas.size()) {
                            mfa.push_back(getArg(mas, map));
                            if (map < mas.size() && mas[map] == ',') map++;
                            else break;
                        }
                        if (!mfa.empty()) {
                            curMethodName = mfa[0];
                            // 保存参数名 (跳过方法名)
                            for (size_t k = 1; k < mfa.size(); k++) {
                                std::string pn = mfa[k]; trim(pn);
                                if (!pn.empty()) curMethodParams.push_back(pn);
                            }
                            inMethod = true;
                        }
                    }
                ...
                if (l == "EndFunc" || l == "endif" || l == "endfor" ||
                    l == "endwhile" || l == "endTry" || l == "endThread") depth--;
                if (inMethod) curMethodBody.push_back(lines[j]);
                if (j == lines.size() - 1) i = j;
            }
            if (inMethod && !curMethodBody.empty()) {
                cls.methods[curMethodName] = curMethodBody;
                cls.methodParams[curMethodName] = curMethodParams;
            }
            g_classes[cls.name] = cls;
            Log("[Class] " + cls.name + " (" + std::to_string(cls.fields.size()) +
                " fields, " + std::to_string(cls.methods.size()) + " methods)");
            continue;
        }
        ...
        // continue 语句 (跳过当前迭代,继续下一次)
        if (s == "continue") {
            g_breakLoop = true;  // 复用 break 机制,但设置特殊标志
            return;
        }

        // throw 语句 - 抛出异常
        if (startsWith(s, "throw ")) {
            std::string msg = s.substr(6);
            trim(msg);
            std::string val = resolveFuncArg(msg);
            reportError(ERR_INTERNAL, "throw: " + val);
            return;
        }
        if (s == "throw") {
            reportError(ERR_INTERNAL, "throw");
            return;
        }
...
static const char* g_keywords[] = {
    ...
    "Class","EndClass","new","this","continue","throw","lambda",
