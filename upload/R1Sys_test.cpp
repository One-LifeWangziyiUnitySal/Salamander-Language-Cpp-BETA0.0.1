...
struct Function { string name; vector<string> params; vector<string> body; map<string,string> capturedVars; };
...
struct ClassDef {
    ...
    map<string, vector<string> > methodParams;
};
map<string, ClassDef> g_classes;
string g_this;
bool isClassDef(const string& name) { return g_classes.find(name) != g_classes.end(); }

// ---- 模块/命名空间系统 (Module) ----
set<string> g_modules;
string g_currentModule;
bool g_inModuleBody = false;  // 仅在 Module 块体内为 true (用于变量赋值前缀化)
...
static void expandModuleAccess(string& s);
static bool tryOperatorOverload(const string& expr, string& result);
...
double calc(const string& e){
    ...
    // 模块内: 先尝试 Module__varName
    if(!g_currentModule.empty()&&isIdentifier(s)){string p=g_currentModule+"__"+s;if(hasVar(p))return atof(getVar(p).c_str());}
...
string callFunc(const string& name,const vector<string>& args){
    ...
    // 推断模块名 (从函数名 M__func 提取 M), 用于模块内裸名查找
    string inferredModule;
    {size_t pos=name.find("__");
     if(pos!=string::npos&&pos>0){string prefix=name.substr(0,pos);if(g_modules.count(prefix))inferredModule=prefix;}}
    string savedModule=g_currentModule;g_currentModule=inferredModule;

    map<string,string> savedGlobalVars=g_vars;
    // 闭包变量和参数名集合 (局部变量, 返回时清除)
    set<string> closureAndParams;
    for(auto& cv:f.capturedVars) closureAndParams.insert(cv.first);
    for(auto& p:f.params) closureAndParams.insert(p);
    closureAndParams.insert("__ret__");  // __ret__ 不保留修改 (返回值机制)
    // 闭包: 注入捕获的变量 (在参数绑定之前)
    for(auto& cv:f.capturedVars) g_vars[cv.first]=cv.second;
    ...
    // 保留对已存在全局变量 (非闭包、非参数) 的修改; 清除局部变量
    for(auto& vp:g_vars){if(savedGlobalVars.count(vp.first)&&!closureAndParams.count(vp.first))savedGlobalVars[vp.first]=vp.second;}
    g_vars=savedGlobalVars;
    g_currentModule=savedModule;
    return ret;
}
...
string evalExpr(const string& expr){
    ...
    // 模块访问展开
    expandModuleAccess(s);
    if(isQuoted(s))return parseStringLiteral(s);
    // this 关键字
    if(s=="this"){if(g_this.empty())return"";return g_this;}
    // this.field 访问成员变量 (仅当 field 是简单标识符, 不含运算符)
    if(s.size()>5&&s[0]=='t'&&s[1]=='h'&&s[2]=='i'&&s[3]=='s'&&s[4]=='.'&&!g_this.empty()){
        string field=s.substr(5);trim(field);
        if(field.find_first_of("+-*/=()[]")==string::npos&&isDictVar(g_this)){
            auto&d=getDict(g_this);auto it=d.find(field);return(it!=d.end())?it->second:"";
        }
    }
    // obj.field 访问对象成员
    {size_t dot=s.find('.');
        if(dot!=string::npos&&dot>0&&dot<s.size()-1){
            string leftPart=s.substr(0,dot),rightPart=s.substr(dot+1);
            if(rightPart.find_first_of("+-*/=()[]")==string::npos&&leftPart.find_first_of("+-*/=()[]")==string::npos){
                string objName;
                if(leftPart=="this")objName=g_this;
                else if(hasVar(leftPart))objName=getVar(leftPart);
                if(!objName.empty()&&isDictVar(objName)){auto&d=getDict(objName);auto it=d.find(rightPart);if(it!=d.end())return it->second;}
            }
        }
    }
    if(isIdentifier(s)){
        if(!g_currentModule.empty()){string p=g_currentModule+"__"+s;if(hasVar(p))return getVar(p);if(isListVar(p))return p;if(isDictVar(p))return p;}
        if(hasVar(s))return getVar(s);
        if(isListVar(s))return s;
        if(isDictVar(s))return s;
    }
    ...
    while(replaced&&safetyCount++<1000){replaced=false;
        ...
        if(!replaced){
            static const char* builtins[]={
                ...
                "iter","hasNext","next","iterValue",
                "new","type","isNum","isStr","isList","isDict","isFunc","isClass",
                "lastError","lastErrorCode","clearErrors","errorCount",
                NULL
            };
            for(int bi=0;builtins[bi];bi++){
                ...
                while(pos!=string::npos){
                    ...
                    string trimmedArgs=argsStr;trim(trimmedArgs);
        ...
        // Lambda: lambda(params, body) -> 匿名闭包
        if(!replaced){
            // 变量作为函数调用: f(args) where f is a var containing function name
            for(size_t pos=0; pos<s.size(); pos++){
                if(s[pos]=='(' && pos>0){
                    size_t nameEnd = pos, nameStart = pos;
                    while(nameStart>0 && (isalnum((unsigned char)s[nameStart-1])||s[nameStart-1]=='_')) nameStart--;
                    if(nameStart >= nameEnd) continue;
                    string varName = s.substr(nameStart, nameEnd-nameStart);
                    if(!isIdentifier(varName) || !hasVar(varName)) continue;
                    string funcName = getVar(varName);
                    if(g_funcs.find(funcName)==g_funcs.end()) continue;
                    size_t lp=pos+1; int depth=1; size_t rp=lp; bool inS=false,inD=false;
                    while(rp<s.size()&&depth>0){char c=s[rp];
                        if(inS){if(c=='\'')inS=false;}else if(inD){if(c=='"')inD=false;}
                        else if(c=='\'')inS=true;else if(c=='"')inD=true;
                        else if(c=='(')depth++;else if(c==')'){depth--;if(depth==0)break;}
                    ...
                    while(ap<=argsStr.size()){string a=getArg(argsStr,ap);cargs.push_back(evalExpr(a));
                        if(g_waitingInput)return "";if(ap<argsStr.size()&&argsStr[ap]==',')ap++;else break;}
                    string fr=callFunc(funcName,cargs);
                    if(g_waitingInput)return "";
                    s.replace(nameStart,rp-nameStart+1,fr);replaced=true;break;
                }
            }
        }
        if(!replaced){
            size_t lpos=s.find("lambda(");
            if(lpos!=string::npos&&(lpos==0||(lpos>0&&!isalnum((unsigned char)s[lpos-1])&&s[lpos-1]!='_'))){
                size_t lp=lpos+7;int depth=1;size_t rp=lp;bool inS=false,inD=false;
                while(rp<s.size()&&depth>0){char c=s[rp];
                    if(inS){if(c=='\'')inS=false;}else if(inD){if(c=='"')inD=false;}
                    else if(c=='\'')inS=true;else if(c=='"')inD=true;
                    else if(c=='(')depth++;else if(c==')'){depth--;if(depth==0)break;}
                ...
                if(depth==0){
                    string argsStr=s.substr(lp,rp-lp);vector<string> largs;size_t lap=0;
                    while(lap<=argsStr.size()){string a=getArg(argsStr,lap);largs.push_back(evalExpr(a));
                        if(lap<argsStr.size()&&argsStr[lap]==',')lap++;else break;}
                    if(largs.size()>=2){
                        static int lambdaCounter=0;lambdaCounter++;
                        string fnName="__lambda_"+to_string(lambdaCounter)+"__";
                        Function lf;lf.name=fnName;
                        {istringstream iss(largs[0]);string pn;while(getline(iss,pn,',')){trim(pn);if(!pn.empty())lf.params.push_back(pn);}}
                        {istringstream biss(largs[1]);string line;while(getline(biss,line)){trim(line);if(!line.empty())lf.body.push_back(line);}}
                        lf.capturedVars=g_vars;  // 闭包: 捕获当前作用域
                        g_funcs[fnName]=lf;
                        s.replace(lpos,rp-lpos+1,fnName);replaced=true;
                    }
                }
            }
        }
        ...
        if(!replaced){
            for(auto&clsPair:g_classes){
                for(auto&methodPair:clsPair.second.methods){
                    string pattern=methodPair.first+"(";
                    size_t pos=s.find(pattern);
                    while(pos!=string::npos){
                        if(pos<2||s[pos-1]!='.'){pos=s.find(pattern,pos+1);continue;}
                        size_t nameStart=pos-1;
                        while(nameStart>0&&(isalnum((unsigned char)s[nameStart-1])||s[nameStart-1]=='_'))nameStart--;
                        string objVarName=s.substr(nameStart,pos-1-nameStart);
                        if(objVarName.empty()){pos=s.find(pattern,pos+1);continue;}
                        string objDictName;
                        if(objVarName=="this")objDictName=g_this;
                        else if(hasVar(objVarName))objDictName=getVar(objVarName);
                        if(objDictName.empty()||!isDictVar(objDictName)){pos=s.find(pattern,pos+1);continue;}
                        auto&d=getDict(objDictName);
                        auto classIt=d.find("__class__");
                        if(classIt==d.end()||classIt->second!=clsPair.first){pos=s.find(pattern,pos+1);continue;}
                        size_t lp=pos+pattern.length();int depth=1;size_t rp=lp;bool inS=false,inD=false;
                        while(rp<s.size()&&depth>0){char c=s[rp];
                            if(inS){if(c=='\'')inS=false;}else if(inD){if(c=='"')inD=false;}
                            else if(c=='\'')inS=true;else if(c=='"')inD=true;
                        ...
                        while(ap<=argsStr.size()){string a=getArg(argsStr,ap);cargs.push_back(evalExpr(a));
                            if(g_waitingInput)return"";if(ap<argsStr.size()&&argsStr[ap]==',')ap++;else break;}
                        string savedThis=g_this;g_this=objDictName;
                        map<string,string> savedVars=g_vars;
                        auto&params=clsPair.second.methodParams[methodPair.first];
                        for(size_t ai=0;ai<cargs.size()&&ai<params.size();ai++)setVar(params[ai],cargs[ai]);
                        setVar("__ret__","");bool savedRet=g_returning;g_returning=false;
                        bool savedBreak=g_breakLoop;g_breakLoop=false;
                        runLines(methodPair.second);
                        string fr=getVar("__ret__");
                        g_returning=savedRet;g_breakLoop=savedBreak;g_vars=savedVars;g_this=savedThis;
                        s.replace(nameStart,rp-nameStart+1,fr);replaced=true;break;
                    }
                    if(replaced)break;
                }
    ...
    // 运算符重载: 如果含 +,-,*,/ 且操作数是对象, 尝试调用 __add__ 等方法
    if(s.find_first_of("+-*/")!=string::npos){
        string olResult;
        if(tryOperatorOverload(s,olResult))return olResult;
    }
    // 替换 this.field 和 obj.field 为其值 (使 calc 能处理含字段访问的算术表达式)
        ...
        while(didReplace&&safety++<1000){didReplace=false;
            for(size_t p=0;p<s.size();p++){
                if(s[p]=='.'&&p>0){
                    ...
                    while(nameStart>0&&(isalnum((unsigned char)s[nameStart-1])||s[nameStart-1]=='_'))nameStart--;
                    if(nameStart>=p)continue;
                    string objName2=s.substr(nameStart,p-nameStart);
                    if(!isIdentifier(objName2))continue;
                    // 向后找字段名
                    size_t fieldEnd=p+1;
                    while(fieldEnd<s.size()&&(isalnum((unsigned char)s[fieldEnd])||s[fieldEnd]=='_'))fieldEnd++;
                    if(fieldEnd<=p+1)continue;
                    string field=s.substr(p+1,fieldEnd-p-1);
                    // 获取对象 dict 名
                    string odn;
                    if(objName2=="this")odn=g_this;
                    else if(hasVar(objName2))odn=getVar(objName2);
                    else continue;
                    if(odn.empty()||!isDictVar(odn))continue;
                    auto&d=getDict(odn);auto it=d.find(field);
                    if(it==d.end())continue;
                    string val=it->second;
                    if(!isPureNumber(val))val="\""+val+"\"";
                    s.replace(nameStart,fieldEnd-nameStart,val);didReplace=true;break;
                }
            }
        }
        trim(s);if(s.empty())return"";
        if(isQuoted(s))return parseStringLiteral(s);
        if(isPureNumber(s))return s;
    }
    ...
    if(hasPlus){size_t pp;
        if(findConcatPlus(s,pp)){
            ...
            // 求值两边 (支持 this.field, obj.field 等复杂表达式)
            string lv=evalExpr(lt),rv=evalExpr(rt);
            if(g_waitingInput)return"";
            bool lIsStr=false,rIsStr=false;
            if(isQuoted(lt))lIsStr=true;
            else if(!isPureNumber(lv))lIsStr=true;
            if(isQuoted(rt))rIsStr=true;
            else if(!isPureNumber(rv))rIsStr=true;
            if(lIsStr||rIsStr)return lv+rv;
            // 两边都是数值, 做算术加法
            return numToStr(atof(lv.c_str())+atof(rv.c_str()));
        }
    }
...
string resolveFuncArg(const string& raw){
    ...
    if(isQuoted(s))return parseStringLiteral(s);
    if(isIdentifier(s)){
        // 模块内: 先尝试 Module__varName (包括列表/字典)
        if(!g_currentModule.empty()){
            string prefixed=g_currentModule+"__"+s;
            if(hasVar(prefixed))return getVar(prefixed);
            if(isListVar(prefixed))return prefixed;
            if(isDictVar(prefixed))return prefixed;
        }
        if(hasVar(s))return getVar(s);
        if(isListVar(s))return s;
        if(isDictVar(s))return s;
        return s; // 未定义标识符作为字符串
    }
    return evalExpr(s);
}
...
bool builtinCall(const string& name, const vector<string>& args, string& result){
    // 辅助: 模块内解析集合名 (列表/字典)
    auto resolveCollName = [](const string& n) -> string {
        if(!g_currentModule.empty() && isIdentifier(n)) {
            string p = g_currentModule + "__" + n;
            if(g_lists.count(p) || g_dicts.count(p)) return p;
        }
        return n;
    };
    // 先检查 C++ 原生函数
    ...
    // 列表函数
    if(name=="listNew"){ if(args.empty()){result="";return true;} string ln=args[0]; if(g_inModuleBody&&isIdentifier(ln))ln=g_currentModule+"__"+ln; vector<string> elems; for(size_t i=1;i<args.size();i++)elems.push_back(args[i]); setList(ln,elems); result=ln; return true; }
    if(name=="listLen"){ if(args.empty()){result="0";return true;} result=to_string(getList(resolveCollName(args[0])).size()); return true; }
    if(name=="listGet"){ if(args.size()<2){result="";return true;} int i=atoi(args[1].c_str()); string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); if(i<0||i>=(int)lst.size()){result="";return true;} result=lst[i]; return true; }
    if(name=="listSet"){ if(args.size()<3){result="";return true;} int i=atoi(args[1].c_str()); string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); if(i<0||i>=(int)lst.size()){result="";return true;} lst[i]=args[2]; result=args[2]; return true; }
    if(name=="listAppend"){ if(args.size()<2){result="";return true;} string ln=resolveCollName(args[0]); if(!isListVar(ln))setList(ln,vector<string>()); getList(ln).push_back(args[1]); result=args[1]; return true; }
    if(name=="listInsert"){ if(args.size()<3){result="";return true;} int i=atoi(args[1].c_str()); string ln=resolveCollName(args[0]); if(!isListVar(ln))setList(ln,vector<string>()); auto&lst=getList(ln); if(i<0)i=0; if(i>(int)lst.size())i=(int)lst.size(); lst.insert(lst.begin()+i,args[2]); result=args[2]; return true; }
    if(name=="listRemove"){ if(args.size()<2){result="";return true;} int i=atoi(args[1].c_str()); string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); if(i<0||i>=(int)lst.size()){result="";return true;} lst.erase(lst.begin()+i); result="true"; return true; }
    if(name=="listPop"){ if(args.empty()){result="";return true;} string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); if(lst.empty()){result="";return true;} result=lst.back(); lst.pop_back(); return true; }
    if(name=="listClear"){ if(args.empty()){result="";return true;} setList(resolveCollName(args[0]),vector<string>()); result="true"; return true; }
    if(name=="listSort"){ if(args.empty()){result="";return true;} string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); sort(lst.begin(),lst.end()); result="true"; return true; }
    if(name=="listReverse"){ if(args.empty()){result="";return true;} string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="";return true;} auto&lst=getList(ln); reverse(lst.begin(),lst.end()); result="true"; return true; }
    if(name=="listFind"){ if(args.size()<2){result="-1";return true;} string ln=resolveCollName(args[0]); if(!isListVar(ln)){result="-1";return true;} auto&lst=getList(ln); for(size_t i=0;i<lst.size();i++){if(lst[i]==args[1]){result=to_string((int)i);return true;}} result="-1"; return true; }
    if(name=="listContains"){ if(args.size()<2){result="false";return true;} auto&lst=getList(resolveCollName(args[0])); for(auto&v:lst)if(v==args[1]){result="true";return true;} result="false"; return true; }
    if(name=="listCopy"){ if(args.size()<2){result="";return true;} string dst=args[1]; if(g_inModuleBody&&isIdentifier(dst))dst=g_currentModule+"__"+dst; setList(dst,getList(resolveCollName(args[0]))); result=dst; return true; }
    if(name=="listSum"){ if(args.empty()){result="0";return true;} double sum=0; auto&lst=getList(resolveCollName(args[0])); for(auto&v:lst)sum+=atof(v.c_str()); char b[64];sprintf(b,"%g",sum); result=b; return true; }
    if(name=="listJoin"){ if(args.size()<2){result="";return true;} string r; auto&lst=getList(resolveCollName(args[0])); for(size_t i=0;i<lst.size();i++){if(i>0)r+=args[1];r+=lst[i];} result=r; return true; }
    if(name=="listPrint"){ if(args.empty()){result="";return true;} auto&lst=getList(resolveCollName(args[0])); string r="["; for(size_t i=0;i<lst.size();i++){if(i>0)r+=", ";r+=lst[i];} r+="]"; Log(r); result=r; return true; }
    // 常量函数
    ...
    // 字典函数
    if(name=="dictNew"){ if(args.empty()){result="";return true;} string dn=args[0]; if(g_inModuleBody&&isIdentifier(dn))dn=g_currentModule+"__"+dn; setDict(dn,map<string,string>()); result=dn; return true; }
    if(name=="dictSet"){ if(args.size()<3){result="";return true;} string dn=resolveCollName(args[0]); if(!isDictVar(dn))setDict(dn,map<string,string>()); getDict(dn)[args[1]]=args[2]; result=args[2]; return true; }
    if(name=="dictGet"){ if(args.size()<2){result="";return true;} string dn=resolveCollName(args[0]); if(!isDictVar(dn)){result="";return true;} auto&d=getDict(dn); auto it=d.find(args[1]); result=(it!=d.end())?it->second:""; return true; }
    if(name=="dictHas"){ if(args.size()<2){result="false";return true;} string dn=resolveCollName(args[0]); if(!isDictVar(dn)){result="false";return true;} auto&d=getDict(dn); result=(d.find(args[1])!=d.end())?"true":"false"; return true; }
    if(name=="dictRemove"){ if(args.size()<2){result="";return true;} string dn=resolveCollName(args[0]); if(!isDictVar(dn)){result="";return true;} getDict(dn).erase(args[1]); result="true"; return true; }
    if(name=="dictLen"){ if(args.empty()){result="0";return true;} result=to_string(getDict(resolveCollName(args[0])).size()); return true; }
    if(name=="dictClear"){ if(args.empty()){result="";return true;} setDict(resolveCollName(args[0]),map<string,string>()); result="true"; return true; }
    if(name=="dictKeys"){ if(args.empty()){result="";return true;} vector<string> keys; auto&d=getDict(resolveCollName(args[0])); for(auto&kv:d)keys.push_back(kv.first); static int kc=0; kc++; string ln="__dk_"+to_string(kc)+"__"; setList(ln,keys); result=ln; return true; }
    if(name=="dictValues"){ if(args.empty()){result="";return true;} vector<string> vals; auto&d=getDict(resolveCollName(args[0])); for(auto&kv:d)vals.push_back(kv.second); static int vc=0; vc++; string ln="__dv_"+to_string(vc)+"__"; setList(ln,vals); result=ln; return true; }
    if(name=="dictCopy"){ if(args.size()<2){result="";return true;} string dst=args[1]; if(g_inModuleBody&&isIdentifier(dst))dst=g_currentModule+"__"+dst; setDict(dst,getDict(resolveCollName(args[0]))); result=dst; return true; }
    if(name=="dictPrint"){ if(args.empty()){result="";return true;} auto&d=getDict(resolveCollName(args[0])); string r="{"; bool first=true; for(auto&kv:d){if(!first)r+=", ";r+=kv.first+": "+kv.second;first=false;} r+="}"; Log(r); result=r; return true; }
    if(name=="dictMerge"){ if(args.size()<2){result="";return true;} string dn=resolveCollName(args[0]); if(!isDictVar(dn))setDict(dn,map<string,string>()); auto&src=getDict(resolveCollName(args[1])); auto&dst=getDict(dn); for(auto&kv:src)dst[kv.first]=kv.second; result="true"; return true; }
    // 数学函数
    ...
    // list() 和 dict() 字面量构造
    if(name=="list"){ string listName="__lit_list_"+to_string(g_lists.size())+"__"; setList(listName,args); result=listName; return true; }
    if(name=="dict"){ string dictName="__lit_dict_"+to_string(g_dicts.size())+"__"; map<string,string> d; for(size_t i=0;i+1<args.size();i+=2)d[args[i]]=args[i+1]; g_dicts[dictName]=d; result=dictName; return true; }
    // 类型判断
    if(name=="isNum"){ if(args.empty()){result="false";return true;} result=isPureNumber(args[0])?"true":"false"; return true; }
    if(name=="isStr"){ if(args.empty()){result="false";return true;} result=isQuoted(args[0])?"true":"false"; return true; }
    if(name=="isList"){ if(args.empty()){result="false";return true;} result=isListVar(args[0])?"true":"false"; return true; }
    if(name=="isDict"){ if(args.empty()){result="false";return true;} result=isDictVar(args[0])?"true":"false"; return true; }
    if(name=="isFunc"){ if(args.empty()){result="false";return true;} result=(g_funcs.find(args[0])!=g_funcs.end())?"true":"false"; return true; }
    if(name=="isClass"){ if(args.empty()){result="false";return true;} result=isClassDef(args[0])?"true":"false"; return true; }
    if(name=="type"){ if(args.empty()){result="";return true;}
        if(isQuoted(args[0])){result="string";return true;}
        if(isPureNumber(args[0])){result="number";return true;}
        if(isListVar(args[0])){result="list";return true;}
        if(isDictVar(args[0])){auto&d=getDict(args[0]);if(d.count("__class__")){result=d["__class__"];return true;}result="dict";return true;}
        if(g_funcs.count(args[0])){result="function";return true;}
        if(isClassDef(args[0])){result="class";return true;}
        result="string";return true; }
    // 错误信息
    if(name=="lastError"){ if(g_errors.empty()){result="";return true;} result=g_errors.back().message; return true; }
    if(name=="lastErrorCode"){ if(g_errors.empty()){result="0";return true;} result=to_string(g_errors.back().code); return true; }
    if(name=="clearErrors"){ g_errors.clear(); result="true"; return true; }
    if(name=="errorCount"){ result=to_string(g_errors.size()); return true; }
    // new(类名) - 实例化对象
    if(name=="new"){ if(args.empty()){result="";return true;} string clsName=args[0]; if(!isClassDef(clsName)){reportError(ERR_UNDEFINED_FUNC,"undefined class: "+clsName);result="";return true;}
        static int objCounter=0; objCounter++; string objName="__obj_"+to_string(objCounter)+"__";
        map<string,string> obj; obj["__class__"]=clsName; ClassDef&cls=g_classes[clsName];
        for(auto&f:cls.fields) obj[f]="";
        setDict(objName,obj); result=objName; return true; }
    // ============ 迭代器 (Iterator) ============
    static int iterCounter=0;
    if(name=="iter"){ if(args.empty()){result="";return true;} string src=args[0]; iterCounter++;
        string iterName="__iter_"+to_string(iterCounter)+"__"; map<string,string> it;
        if(isListVar(src)){ string dn=iterName+"_data"; setList(dn,getList(src)); it["__type__"]="list"; it["__data__"]=dn; it["__pos__"]="0"; }
        else if(isDictVar(src)){ string dn=iterName+"_data"; setDict(dn,getDict(src)); string kn=iterName+"_keys"; vector<string> keys; for(auto&kv:getDict(src))keys.push_back(kv.first); setList(kn,keys); it["__type__"]="dict"; it["__data__"]=dn; it["__keys__"]=kn; it["__pos__"]="0"; }
        else { result=""; return true; }
        setDict(iterName,it); result=iterName; return true; }
    if(name=="hasNext"){ if(args.empty()||!isDictVar(args[0])){result="false";return true;}
        auto&it=getDict(args[0]); string type=it.count("__type__")?it["__type__"]:""; int pos=it.count("__pos__")?atoi(it["__pos__"].c_str()):0;
        if(type=="list"){string dn=it["__data__"]; if(!isListVar(dn)){result="false";return true;} result=(pos<(int)getList(dn).size())?"true":"false";}
        else if(type=="dict"){string kn=it["__keys__"]; if(!isListVar(kn)){result="false";return true;} result=(pos<(int)getList(kn).size())?"true":"false";}
        else result="false"; return true; }
    if(name=="next"){ if(args.empty()||!isDictVar(args[0])){result="";return true;}
        auto&it=getDict(args[0]); string type=it.count("__type__")?it["__type__"]:""; int pos=it.count("__pos__")?atoi(it["__pos__"].c_str()):0;
        if(type=="list"){string dn=it["__data__"]; if(!isListVar(dn)){result="";return true;} auto&lst=getList(dn); if(pos<0||pos>=(int)lst.size()){result="";return true;} result=lst[pos]; it["__pos__"]=to_string(pos+1);}
        else if(type=="dict"){string kn=it["__keys__"]; if(!isListVar(kn)){result="";return true;} auto&keys=getList(kn); if(pos<0||pos>=(int)keys.size()){result="";return true;} result=keys[pos]; it["__pos__"]=to_string(pos+1);}
        else result=""; return true; }
    if(name=="iterValue"){ if(args.empty()||!isDictVar(args[0])){result="";return true;}
        auto&it=getDict(args[0]); string type=it.count("__type__")?it["__type__"]:""; int pos=it.count("__pos__")?atoi(it["__pos__"].c_str()):0;
        if(type=="dict"){string kn=it["__keys__"],dn=it["__data__"]; if(isListVar(kn)&&isDictVar(dn)){auto&keys=getList(kn);int idx=pos-1;if(idx>=0&&idx<(int)keys.size()){auto&d=getDict(dn);auto kv=d.find(keys[idx]);result=(kv!=d.end())?kv->second:"";}}}
        else if(type=="list"){string dn=it["__data__"]; if(isListVar(dn)){auto&lst=getList(dn);int idx=pos-1;if(idx>=0&&idx<(int)lst.size())result=lst[idx];}}
        return true; }
    return false;
}
...
// 模块访问展开: "ModuleName.member" -> "ModuleName__member"
static void expandModuleAccess(string& s) {
    if (g_modules.empty()) return;
    bool inS = false, inD = false;
    // 第一遍: Module.member → Module__member (限定访问)
    for (size_t i = 0; i < s.size(); i++) {
        ...
        if (c == '.') {
            if (i == 0) continue;
            size_t nameEnd = i, nameStart = i;
            while (nameStart > 0 && (isalnum((unsigned char)s[nameStart-1]) || s[nameStart-1] == '_'))
                nameStart--;
            if (nameStart >= nameEnd) continue;
            string modName = s.substr(nameStart, nameEnd - nameStart);
            if (g_modules.count(modName)) {
                s.replace(i, 1, "__");
                i += 2;
            }
        }
    }
    // 第二遍: 模块内裸函数调用 funcName( → Module__funcName( (非限定访问)
    if (!g_currentModule.empty()) {
        string prefix = g_currentModule + "__";
        for (auto& fp : g_funcs) {
            if (fp.first.size() > prefix.size() &&
                fp.first.compare(0, prefix.size(), prefix) == 0) {
                string shortName = fp.first.substr(prefix.size());
                string pat = shortName + "(";
                size_t pos = 0;
                while ((pos = s.find(pat, pos)) != string::npos) {
                    if (pos > 0 && (isalnum((unsigned char)s[pos-1]) || s[pos-1] == '_')) { pos += 1; continue; }
                    if (pos >= 2 && s[pos-1] == '_' && s[pos-2] == '_') { pos += 1; continue; }
                    s.replace(pos, pat.size(), fp.first + "(");
                    pos += fp.first.size() + 1;
                }
            }
        }
...
// 运算符重载: 检查 +-*/ 操作数是否为对象, 调用 __add__/__sub__/__mul__/__div__
static bool tryOperatorOverload(const string& expr, string& result) {
    string s = expr; trim(s);
    if (s.empty()) return false;
    size_t pos = string::npos; char op = 0;
    if (findTopLevelOp(s, '+', '-', pos)) op = s[pos];
    else if (findTopLevelMulDiv(s, '*', pos)) op = '*';
    else if (findTopLevelMulDiv(s, '/', pos)) op = '/';
    if (pos == string::npos) return false;
    string lhs = s.substr(0, pos), rhs = s.substr(pos + 1);
    trim(lhs); trim(rhs);
    if (lhs.empty() || rhs.empty()) return false;
    string lv = evalExpr(lhs); if (g_waitingInput) return false;
    string rv = evalExpr(rhs); if (g_waitingInput) return false;
    string objDictName, className, otherVal;
    if (!lv.empty() && g_dicts.count(lv)) {
        auto& d = g_dicts[lv]; auto it = d.find("__class__");
        if (it != d.end()) { objDictName = lv; className = it->second; otherVal = rv; }
    }
    if (objDictName.empty() && !rv.empty() && g_dicts.count(rv)) {
        auto& d = g_dicts[rv]; auto it = d.find("__class__");
        if (it != d.end()) { objDictName = rv; className = it->second; otherVal = lv; }
    }
    if (objDictName.empty() || className.empty() || !isClassDef(className)) return false;
    string methodName;
    switch (op) {
        case '+': methodName = "__add__"; break;
        case '-': methodName = "__sub__"; break;
        case '*': methodName = "__mul__"; break;
        case '/': methodName = "__div__"; break;
        default: return false;
    }
    ClassDef& cls = g_classes[className];
    if (cls.methods.find(methodName) == cls.methods.end()) return false;
    string savedThis = g_this; g_this = objDictName;
    map<string,string> savedVars = g_vars;
    auto& params = cls.methodParams[methodName];
    for (size_t ai = 0; ai < params.size(); ai++) setVar(params[ai], ai == 0 ? otherVal : "");
    setVar("__ret__", "");
    bool savedRet = g_returning; g_returning = false;
    bool savedBreak = g_breakLoop; g_breakLoop = false;
    runLines(cls.methods[methodName]);
    result = getVar("__ret__");
    g_returning = savedRet; g_breakLoop = savedBreak;
    g_vars = savedVars; g_this = savedThis;
    return true;
}
...
void runCode(const string& input){
    ...
    if(eqPos!=string::npos&&eqPos>0&&(parenPos==string::npos||eqPos<parenPos)){
        ...
        // obj.field = val 或 this.field = val 赋值
        {
            size_t dot=vn.find('.');
            if(dot!=string::npos&&dot>0&&dot<vn.size()-1){
                string leftPart=vn.substr(0,dot),field=vn.substr(dot+1);trim(leftPart);trim(field);
                string objName;
                if(leftPart=="this")objName=g_this;
                else if(hasVar(leftPart))objName=getVar(leftPart);
                if(!objName.empty()&&isDictVar(objName)){
                    string val=resolveFuncArg(vl);if(g_waitingInput)return;
                    getDict(objName)[field]=val;return;
                }
            }
        }
        if(!isIdentifier(vn)){reportError(ERR_INVALID_NAME,"invalid variable name: "+vn);return;}
        {string val=resolveFuncArg(vl);if(g_waitingInput)return;
         if(g_inModuleBody && vn.find("__")==string::npos)vn=g_currentModule+"__"+vn;
         setVar(vn,val);}return;
    }
...
void runLines(const vector<string>& lines){
    for(size_t i=0;i<lines.size();i++){
        ...
        if(startsWith(s,"Func(")){
            ...
            if(!g_currentModule.empty())func.name=g_currentModule+"__"+func.name;
            g_funcs[func.name]=func;continue;
        }
        // Module(模块名) ... EndModule
        if(startsWith(s,"Module(")){
            size_t p1=s.find('('),p2=s.find_last_of(')');
            if(p1!=string::npos&&p2!=string::npos&&p2>p1){
                string modName=s.substr(p1+1,p2-p1-1);trim(modName);modName=resolveFuncArg(modName);
                if(!modName.empty()){
                    // 嵌套模块: 拼接前缀 (Outer__Inner)
                    string fullModName = modName;
                    if(!g_currentModule.empty()) fullModName = g_currentModule + "__" + modName;
                    g_modules.insert(fullModName);
                    string savedModule=g_currentModule;g_currentModule=fullModName;
                    bool savedInModuleBody=g_inModuleBody;g_inModuleBody=true;
                    int depth=0;vector<string> modBody;
                    for(size_t j=i+1;j<lines.size();j++){string l=lines[j];trim(l);
                        if(startsWith(l,"Module("))depth++;
                        if(l=="EndModule"){if(depth==0){i=j;break;}depth--;}
                        modBody.push_back(lines[j]);if(j==lines.size()-1)i=j;
                    }
                    runLines(modBody);
                    g_currentModule=savedModule;
                    g_inModuleBody=savedInModuleBody;
                }
            }
            continue;
        }
        ...
        if(startsWith(s,"import ")){
            string modName=s.substr(7);trim(modName);string prefix=modName+"__";
            vector<pair<string,Function> > newFuncs;
            for(auto&fp:g_funcs){if(fp.first.size()>prefix.size()&&fp.first.compare(0,prefix.size(),prefix)==0){string sn=fp.first.substr(prefix.size());Function al=fp.second;al.name=sn;newFuncs.push_back(make_pair(sn,al));}}
            for(auto&nf:newFuncs)g_funcs[nf.first]=nf.second;
            map<string,string> newVars;
            for(auto&vp:g_vars){if(vp.first.size()>prefix.size()&&vp.first.compare(0,prefix.size(),prefix)==0){string sn=vp.first.substr(prefix.size());newVars[sn]=vp.second;}}
            for(auto&nv:newVars)g_vars[nv.first]=nv.second;
            continue;
        }
        // Class(类名, 成员变量...) ... EndClass
        if(startsWith(s,"Class(")){
            size_t p1=s.find('('),p2=s.find_last_of(')');
            if(p1!=string::npos&&p2!=string::npos){
                string as=s.substr(p1+1,p2-p1-1);vector<string> ca;size_t ap=0;
                while(ap<=as.size()){ca.push_back(getArg(as,ap));if(ap<as.size()&&as[ap]==',')ap++;else break;}
                if(!ca.empty()){
                    ClassDef cls;cls.name=ca[0];
                    for(size_t k=1;k<ca.size();k++){string fn=ca[k];trim(fn);if(!fn.empty())cls.fields.push_back(fn);}
                    int depth=0;string curMethodName;vector<string> curMethodParams;vector<string> curMethodBody;bool inMethod=false;
                    for(size_t j=i+1;j<lines.size();j++){string l=lines[j];trim(l);
                        if(l=="EndClass"&&depth==0){i=j;break;}
                        if(startsWith(l,"Func(")&&depth==0){
                            if(inMethod){cls.methods[curMethodName]=curMethodBody;cls.methodParams[curMethodName]=curMethodParams;curMethodBody.clear();curMethodParams.clear();}
                            size_t mp1=l.find('('),mp2=l.find_last_of(')');
                            if(mp1!=string::npos&&mp2!=string::npos){
                                string mas=l.substr(mp1+1,mp2-mp1-1);vector<string> mfa;size_t map=0;
                                while(map<=mas.size()){mfa.push_back(getArg(mas,map));if(map<mas.size()&&mas[map]==',')map++;else break;}
                                if(!mfa.empty()){curMethodName=mfa[0];for(size_t k=1;k<mfa.size();k++){string pn=mfa[k];trim(pn);if(!pn.empty())curMethodParams.push_back(pn);}inMethod=true;}
                            }
                            continue;
                        }
                        if(l=="EndFunc"&&inMethod&&depth==0){cls.methods[curMethodName]=curMethodBody;cls.methodParams[curMethodName]=curMethodParams;curMethodBody.clear();curMethodParams.clear();inMethod=false;continue;}
                        if(startsWith(l,"Func(")||startsWith(l,"if(")||startsWith(l,"For(")||startsWith(l,"while(")||startsWith(l,"try")||startsWith(l,"Thread"))depth++;
                        if(l=="EndFunc"||l=="endif"||l=="endfor"||l=="endwhile"||l=="endTry"||l=="endThread")depth--;
                        if(inMethod)curMethodBody.push_back(lines[j]);
                        if(j==lines.size()-1)i=j;
                    }
                    if(inMethod&&!curMethodBody.empty()){cls.methods[curMethodName]=curMethodBody;cls.methodParams[curMethodName]=curMethodParams;}
                    g_classes[cls.name]=cls;
                }
            }
            continue;
        }
...
void test_closures(){
    ...
    // 基本闭包: lambda 捕获外部变量
    {
        ...
        runLines({
            "n = 10",
            "f = lambda(\"x\", \"return x + n\")",
            "result = f(5)"
        });
        CHECK_EQ(getVar("result"), "15", "闭包: lambda 捕获外部变量 n=10, f(5)=15");
        reset();
    }
    // 闭包捕获后修改原变量不影响闭包 (值快照)
    {
        ...
        CHECK_EQ(getVar("result"), "15", "闭包: 捕获的是快照, 修改原变量后仍为15");
        reset();
    }
    // 工厂函数: 返回闭包
    {
        ...
        runLines({
            "Func(makeAdder, base)",
            "    return lambda(\"x\", \"return x + base\")",
            "EndFunc",
            "adder10 = makeAdder(10)",
            "adder20 = makeAdder(20)",
            "r1 = adder10(5)",
            "r2 = adder20(5)"
        });
        CHECK_EQ(getVar("r1"), "15", "闭包工厂: makeAdder(10)(5)=15");
        CHECK_EQ(getVar("r2"), "25", "闭包工厂: makeAdder(20)(5)=25");
        reset();
    }
    // 闭包捕获多个变量
    {
        ...
        runLines({
            "a = 3",
            "b = 4",
            "f = lambda(\"x\", \"return x * a + b\")",
            "result = f(10)"
        });
        CHECK_EQ(getVar("result"), "34", "闭包: 捕获多变量 a=3,b=4, f(10)=34");
        reset();
    }
    // 闭包参数优先于捕获变量
    {
        ...
        CHECK_EQ(getVar("result"), "14", "闭包: 参数 x 覆盖捕获的 x=100, f(7)=14");
        reset();
    }
}
...
void test_modules(){
    ...
    // 基本模块: 定义和点号访问
    {
        ...
        CHECK_EQ(getVar("r1"), "7", "模块: Math.add(3,4)=7");
        CHECK_EQ(getVar("r2"), "30", "模块: Math.mul(5,6)=30");
        reset();
    }
    // 模块变量
    {
        ...
        CHECK_EQ(getVar("r"), "3", "模块: Config.PI=3");
        reset();
    }
    // import 导入
    {
        ...
        CHECK_EQ(getVar("r"), "30", "模块: import Math 后直接调用 add(10,20)=30");
        reset();
    }
    // 模块隔离: 模块内变量不影响全局
    {
        ...
        CHECK_EQ(getVar("r"), "1", "模块: 模块内 x 不污染全局 x=1");
        reset();
    }
    // 模块内函数直接调用 (不限定)
    {
        ...
        CHECK_EQ(getVar("r"), "10", "模块: 函数直接调用 g(n) M.f(5)=10");
        reset();
    }
    // 模块内函数访问模块变量 (直接)
    {
        ...
        CHECK_EQ(getVar("r"), "12", "模块: 函数访问变量 PI M.area(2)=12");
        reset();
    }
    // 模块内递归 (直接调用)
    {
        ...
        CHECK_EQ(getVar("r"), "120", "模块: 递归 fact(5)=120");
        reset();
    }
    // 模块内修改变量 (持久化)
    {
        ...
        CHECK_EQ(getVar("r1"), "1", "模块: 持久化修改 (1) M.inc()=1");
        CHECK_EQ(getVar("r2"), "2", "模块: 持久化修改 (2) M.inc()=2");
        CHECK_EQ(getVar("r3"), "3", "模块: 持久化修改 (3) M.inc()=3");
        reset();
    }
    // 模块内修改全局变量
    {
        ...
        CHECK_EQ(getVar("r"), "999", "模块: 函数修改全局变量=999");
        reset();
    }
    // 嵌套模块
    {
        ...
        CHECK_EQ(getVar("r"), "42", "模块: 嵌套模块 Outer.Inner.foo()=42");
        reset();
    }
    // 模块内定义列表
    {
        ...
        CHECK_EQ(getVar("r"), "3", "模块: 模块内列表 listLen(M.data)=3");
        reset();
    }
    // 模块内定义字典
    {
        ...
        CHECK_EQ(getVar("r"), "Alice", "模块: 模块内字典 dictGet(M.d)=Alice");
        reset();
    }
    // 模块函数返回列表
    {
        ...
        CHECK_EQ(getVar("r"), "5", "模块: 函数返回列表 listLen=5");
        reset();
    }
}

// ============================================================
// 运算符重载 (Operator Overloading) 测试
// ============================================================
void test_operator_overloading(){
    cout<<"\n=== [运算符重载 Operator Overloading] ==="<<endl;
    // __add__ 重载
    {
        reset();
        runLines({
        ...
        CHECK_EQ(getVar("rx"), "13", "运算符重载: Vec(3,4)+Vec(10,20).x=13");
        CHECK_EQ(getVar("ry"), "24", "运算符重载: Vec(3,4)+Vec(10,20).y=24");
        reset();
    }
    // __mul__ 重载
    {
        ...
        CHECK_EQ(getVar("r"), "42", "运算符重载: Scalar(6)*Scalar(7)=42");
        reset();
    }
    // 无重载时回退到普通算术
    {
        ...
        CHECK_EQ(getVar("r"), "7", "运算符重载: 无重载时正常算术 3+4=7");
        reset();
    }
}
...
void test_iterators(){
    ...
    // 列表迭代器
    {
        ...
        CHECK_EQ(getVar("s"), "abc", "迭代器: 列表遍历 abc");
        reset();
    }
    // 列表迭代器求和
    {
        ...
        CHECK_EQ(getVar("sum"), "100", "迭代器: 列表求和 10+20+30+40=100");
        reset();
    }
    // 空列表迭代器
    {
        ...
        CHECK_EQ(getVar("r"), "false", "迭代器: 空列表 hasNext=false");
        reset();
    }
    // hasNext 不推进位置
    {
        ...
        CHECK_EQ(getVar("h1"), "true", "迭代器: hasNext 不推进 (第1次)");
        CHECK_EQ(getVar("h2"), "true", "迭代器: hasNext 不推进 (第2次)");
        CHECK_EQ(getVar("v"), "1", "迭代器: next 返回第1个元素");
        reset();
    }
    // 字典迭代器 (遍历键)
    {
        ...
        // 字典键顺序由 map 决定 (age < name)
        CHECK_EQ(getVar("k1"), "age", "迭代器: 字典第1个键=age");
        CHECK_EQ(getVar("v1"), "30", "迭代器: 字典 age=30");
        CHECK_EQ(getVar("k2"), "name", "迭代器: 字典第2个键=name");
        CHECK_EQ(getVar("v2"), "Alice", "迭代器: 字典 name=Alice");
        reset();
    }
}
