...
- [语言手册](#语言手册)
  ...
  - [控制结构](#控制结构)
  - [函数定义](#函数定义)
  - [匿名函数与闭包](#匿名函数与闭包-lambda--closure)
  - [模块 / 命名空间](#模块--命名空间-module)
  - [运算符重载](#运算符重载-operator-overloading)
  - [迭代器](#迭代器-iterator)
  - [输入输出](#输入输出)
  - [列表 List](#列表-list)
  - [字典 Dict](#字典-dict)
...
| 组件 | 说明 |
|------|------|
| **语言标准** | [SLS v1.0](SLS-v1.0.md) — 形式化语言规范 (1861 行) |
...
### 匿名函数与闭包 (Lambda & Closure)

```python
# Lambda 创建匿名函数: lambda(参数列表, 函数体)
f = lambda("x", "return x * 2")
PrintLog(f(21))            # 42

# 闭包: 捕获创建时的变量 (值快照)
n = 10
g = lambda("x", "return x + n")
PrintLog(g(5))             # 15
n = 100
PrintLog(g(5))             # 仍为 15 (捕获的是快照)
...
Func(apply, f, val)
  return f(val)
EndFunc
PrintLog(apply(lambda("x", "return x * x"), 7))  # 49
```
...
# 通过 Module.member 访问
PrintLog(Math.add(3, 4))   # 7
PrintLog(Math.mul(5, 6))   # 30
PrintLog(Math.PI)          # 3

# import 导入到当前作用域
import Math
PrintLog(add(10, 20))      # 30 (直接调用)
...
it = iter(lst)
while(hasNext(it))
  PrintLog(next(it))       # a, b, c, d
endWhile

# 求和
nums = list(10, 20, 30, 40)
it = iter(nums)
total = 0
while(hasNext(it))
  total = total + next(it)
...
while(hasNext(it))
  key = next(it)
  val = iterValue(it)
  PrintLog(key, "=", val)  # age = 30, name = Alice
endWhile

# 函数: iter(collection), hasNext(it), next(it), iterValue(it)
```
...
```
R1Sys.cpp           主程序 (解释器 + IDE, ~5100 行)
R1Sys_test.cpp      测试套件 (878 个测试)
SLS-v1.0.md         SimpleIDE 语言标准 v1.0 (1861 行)
README.md           项目说明
...
| 版本 | 主要功能 |
|------|---------|
| v3.0 | **闭包(Closure)、模块(Module/import)、运算符重载(__add__/__sub__/__mul__/__div__)、迭代器(iter/hasNext/next/iterValue)** |
